...
public class TodosResourceClient {
	...
	public static void main(String[] args) {
		...
		Todo todo = new Todo("5", "Blablabla bla bla");
		...
		System.out.println(target.path("rest").path("todos/2")
				.request(MediaType.APPLICATION_XML).get(String.class));
		// Delete the Todo with id 1
		target.path("rest").path("todos/2").request().delete();
		System.out.println(response.getStatus());
		System.out.println(response.getStatusInfo().toString());
	...
	private static URI getBaseURI() {
		return UriBuilder.fromUri("http://localhost:8080/RESTful_ex").build();
	}
}
